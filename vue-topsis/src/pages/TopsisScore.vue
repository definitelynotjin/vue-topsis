<script setup lang="ts">
  ...
  const saveRankingReport = async () => {
    ...
    try {
      loading.value = true
      const res = await topsisStore.saveRankingReport(projectStore.selectedProjectId)
    ...
    <main>
      <v-container fluid class="bg-cyan-700 score-container">
        ...
        <template v-slot:bottom="{ props }">
          ...
          <div class="pa-4">
            <p>Data dihitung: {{ topsisResults.total }}</p>
            <p>Data tidak dihitung: {{ topsisResults.skipped }}</p>
          </div>
        </template>
